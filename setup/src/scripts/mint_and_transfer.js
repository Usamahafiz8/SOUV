//Mints a transferable Platform NFT and transfers it to a recipient.
...
async function mintAndTransfer(eventId, supplyCapId, recipientAddress) {
    try {
        const tx = new TransactionBlock();
        tx.moveCall({
            target: `${packageId}::moments::mint_and_transfer`,
            arguments: [
                tx.object(eventId),
        ...
        console.log('Minted and transferred:', result);
        return result;
    } catch (error) {
...
// Example usage
// mintAndTransfer('EVENT_ID', 'SUPPLY_CAP_ID', 'RECIPIENT_ADDRESS');